...
pub fn combined_message_attrs_impl(
    attr: TokenStream,
    ...
    input
        .attrs
        .push(syn::parse_quote! { #[derive(ToDiscriminant, TransitiveChild)] });
    input
        .attrs
        .push(syn::parse_quote! { #[parent(#parent, #variant)] });
    if parent_is_top {
    ...
    input
        .attrs
        .push(syn::parse_quote! { #[discriminant_attr(parent(#parent_discriminant, #variant))] });
    if parent_is_top {
        input
            .attrs
            .push(syn::parse_quote! { #[discriminant_attr(parent_is_top)] });
    }
...
fn top_level_impl(input_item: TokenStream) -> syn::Result<TokenStream> {
    ...
    input
        .attrs
        .push(syn::parse_quote! { #[derive(ToDiscriminant)] });
